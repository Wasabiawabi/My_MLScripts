...
**閾値関数**$σ(z)$は、総入力$z$と**閾値**$θ$を基に0か1を出力する。
閾値は、神経細胞の発火しやすさを表していて、出力0は発火していない状態、出力1は発火した状態と言い換えられる。
パーセプトロンの学習では、閾値関数の閾値が、重みベクトルと同様、最適値を求めたいパラメータとなる。
$$
決定関数 \ \ σ(z_m) =
\begin{cases}
1,\ \ (z_m \ge θ) \\
...
ここで、$\hat y_i$を入力$x_i$と総入力$z$から得られた**予測値**(出力)とすると、**予測値ベクトル**は$\hat y$となり、正解データ$y$と同じ形状になる。
予測値は、パラメータ更新前の重みベクトル$w$、バイアス$b$と入力データから得られる総入力$z$を、閾値関数$σ$に代入することで得られる。
...
2. 入力データ$x_{i}$(1行)ごとに総入力$z$を計算して、閾値関数$σ$から出力値(予測値)$\hat y_i$を得る
...
##### 活性化関数$σ$
パーセプトロンの閾値関数とADALINEの活性化関数は、どちらも**入力に基づいた出力**だが、大きな違いがある。
パーセプトロンの閾値関数とは異なり、ADALINEの活性化関数は**微分によって勾配(=微分値)を求めることができる**。
**なぜ微分できる必要があるのか**を考える。
...
パーセプトロンの閾値関数は連続ではないため、微分が出来ない。
活性化関数は、微分が出来ない閾値関数の代わりとして、損失関数への入力を求める役割があると捉えられる。
また、閾値関数は予測値を求めるときのみ用いられることから、**閾値関数の役割は予測値に変換することに限定される**と考えられる(パラメータの更新に用いるかは別問題ということ)。
ちなみに、現在では活性化関数は複数存在するが、全てADALINEが基礎となっていると見なすことができる。
$$
\begin{gather}
活性化関数(最もシンプル)\\
...
\end{gather}
$$
$w$についての偏導関数の式がちょっと曖昧で正確かどうか怪しいが、正解値と活性化関数からの誤差のベクトルと入力の各列の内積から、ベクトルが算出される。
### ADALINEの実装
##### データ構造
$i$行$j$列の入力データと、行ごとの正解データが与えられるとする。
重みベクトル$w$とバイアス$b$を学習で更新したい。
...
$$
##### 学習則
総入力ベクトル$z$は、正解データ$y$と同じ形状になり、活性化関数に通したベクトルも同じ形状になる。
$error$を正解データ$y$と活性化関数の出力ベクトル$σ(z)$のから得られた**損失ベクトル**とすると、正解データ$y$と同じ形状になる。
$\nabla_wL$について、$1$行$i$列である$error^T$と$i$行$j$列である入力ベクトル$x$のドット積は$1$行$j$列となり、これが重みベクトル$w$と同じ形状になる。
$$
\begin{gather}
総入力ベクトル\\
z=
...
\end{gather}
$$
予測値を得る際は、活性化関数を計算した後、閾値関数に通す。
### プログラムに適用する
パーセプトロンとの違いは、更新値の計算式が損失関数を用いた勾配になっていることと、誤差の更新に閾値関数ではなく活性化関数が用いられていること。
予測値を得る時は、パーセプトロンと同様に閾値関数を用いる。
# パーセプトロンとADALINEの比較
...
パーセプトロンは、線形分類問題では、必ず一定の線引きができるが、非線形分類問題では、「間違えた」という結果に影響されつづけることが原因で、一定の判断基準を求められない。
一方ADALINEは、「誤差が最も少ない線引き」を目指すため、極端な外れ値に引っ張られてしまうことがある反面、どんなデータでも一定の判断基準を求められる。
現実の問題は非線形分類問題の方が多く、パーセプトロンよりはADALINEの方が使い道が多いため、ADALINEはパーセプトロンの改良版と見なすことができる。
